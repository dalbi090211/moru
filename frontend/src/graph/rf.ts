...
import type { GraphNode } from "./commands.ts";
 ...
 */
export const toRFNode = (n: GraphNode): RFNode => {
  const { id, type, ui, ...params } = n;
  return { id, type, position: ui ?? { x: 0, y: 0 }, data: { params } };
};
 ...
 */
export const syncRFNodes = (prev: RFNode[], nodes: GraphNode[]): RFNode[] => {
  const kept = new Map(prev.map((n) => [n.id, n]));
  return nodes.map((n) => {
    const old = kept.get(n.id);
    return old ? { ...old, ...toRFNode(n) } : toRFNode(n);
  });
};
